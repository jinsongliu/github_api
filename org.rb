...
def get_team_members(id)
    puts 'Github user id, full name, details'
    ...
    members.each { |m|
        ...
        user = user['users'][0]
        puts "#{user['username']}, #{user['fullname']}, #{String(user['location']).gsub(',','-')}"
    }
end
...
get_repos_for_org('org') { |org, repo| get_teams_for_repo org, repo}
